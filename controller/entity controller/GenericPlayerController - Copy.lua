	...
	self.entityOutputMappingMethods = {
					...
					if self.movementInputMapper:getCurrentMovementRotation() then
						...
						if not stateComponent.componentTable.movement.state then
							...
							controllerSystem:sendMovementActionRequest(self.MOVEMENT_REQUEST.START_MOVEMENT_CUSTOM, 
								movementComponent, stateComponent.componentTable.combat.action.variables.walkAnimationSetId,
						...
						controllerSystem:sendIdleActionRequest(self.IDLE_REQUEST.START_IDLE_CUSTOM, 
							stateComponent.componentTable.idle, stateComponent.componentTable.combat.action.variables.idleActionSetId,
		...
		[self.OUTPUT_ACTION.COMBAT] = function(self, controllerSystem, stateComponent)
			if self:isActionAllowed(stateComponent.state, self.ENTITY_ACTION.COMBAT) then
				stateComponent.state = self.ENTITY_STATE.COMBAT_FREE
		...
		[self.OUTPUT_ACTION.IDLE] = function(self, controllerSystem, stateComponent)
			...
			if not stateComponent.componentTable.movement.state then
				if stateComponent.state == self.ENTITY_STATE.COMBAT_FREE then
					if not stateComponent.componentTable.combat.action or 
						stateComponent.componentTable.combat.action.variables.walk then
		...
		[self.OUTPUT_ACTION.COMBAT_MOVEMENT] = function(self, controllerSystem, stateComponent)
			
		end,
		
		[self.OUTPUT_ACTION.COMBAT_IDLE] = function(self, controllerSystem, stateComponent)
